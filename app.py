...
df.columns = df.columns.str.replace('\u00a0', ' ')  # Remplace les espaces insécables par des espaces normaux
df.columns = df.columns.str.replace('\t', '') # Supprime les tabulations si présentes

# Vérifier les noms nettoyés
print("Colonnes après nettoyage :", df.columns.tolist())

# 6. Remplacer tous les NaN par une chaîne vide
df = df.fillna('')
...
# Calcul de la durée d'inspection (en minutes)
df['Duree_Inspection'] = (df['Fin_Inspection'] - df['Debut_Inspection']).dt.total_seconds() / 60

# Ajout d’une chaîne lisible pour affichage
...
# Étape 1 : Modèle IsolationForest
...
# Étape 2 : Ajouter règle métier pour forcer certains cas en "Suspecte"
...
# Liste des points de conformité majeurs (à adapter selon les noms de colonnes si disponibles)
points_essentiels = ['prise de terre', 'différentiel', 'disjoncteur de branchement']

# Exemple hypothétique : Tu dois avoir une colonne indiquant les remarques de l’inspecteur (ex: "Non conforme - prise de terre non posée")
...
# ➕ Classification durée inspection
def classer_duree(duree):
    if pd.isna(duree):
        return "Inconnue"
    elif duree < 10:
...
app.layout = html.Div([
    ...
    html.H4("🕵️ Alerte durée d’inspection - Simulation de saisie"),
    html.Label("Sélectionnez un inspecteur :"),
    dcc.Dropdown(
    ...
    html.H4("Cas de fraude détectés (Inspecteur VS Référent)"),
    dcc.Graph(figure=px.bar(df[df['Fraude'] == "Fraude"],
                        x='Inspecteur',
                        title="Cas de fraude détectés par Inspecteur",
                        labels={'Inspecteur': 'Inspecteur'})),

    html.H4("Détails des cas suspects de clôture conforme malgré anomalie"),
    ...
    html.Div([
        html.H4(f"Nombre total de fraudes détectées : {df[df['Fraude'] == 'Fraude'].shape[0]}")
    ]),

    html.H4("🕵️ Saisie de rapport d’inspection - Simulation"),
    html.Label("Sélectionnez un inspecteur :"),
    dcc.Dropdown(
...
# 🔁 Callback pour le graphe des anomalies par mois
@app.callback(
...
def afficher_anomalies(mois):
    ...
    fig = px.scatter(df_mois, x='Debut_Inspection', y='Duree_Inspection', color='Anomalie',
                     title=f"Détection d'anomalies - {mois}",
                     labels={"Duree_Inspection": "Durée (min)", "Debut_Inspection": "Début Inspection"})
    return fig

# 🔁 Callback 1 – pour alerte durée inspection (dropdown 1)
@app.callback(
...
def verifier_duree(inspecteur):
    ...
    if duree < 10:
        couleur = 'red'
        message = f"⚠️ Durée trop courte ({duree:.2f} min) — Inspection suspecte !"
    elif duree > 1000:
        couleur = 'orange'
        message = f"⚠️ Durée trop longue ({duree:.2f} min) — Inspection anormale !"
    else:
        couleur = 'green'
        message = f"✅ Durée normale ({duree:.2f} min)."
    return html.Div(message, style={'color': couleur, 'fontWeight': 'bold'})

# 🔁 Callback 2 – pour alerte saisie rapport (dropdown 2)
@app.callback(
...
def verifier_duree_saisie(inspecteur):
    ...
    if duree < 10:
        couleur = 'red'
        message = f"⚠️ Durée d’inspection trop courte ({duree:.2f} min). Veuillez vérifier."
    elif duree > 1000:
        couleur = 'orange'
        message = f"⚠️ Durée d’inspection très longue ({duree:.2f} min). Cela semble anormal."
    else:
        couleur = 'green'
        message = f"✅ Durée d’inspection normale ({duree:.2f} min)."
    return html.Div(message, style={'color': couleur, 'fontWeight': 'bold'})
